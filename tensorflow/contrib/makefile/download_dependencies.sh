...
# Temporary fix to refer to Mehdi Goli's Eigen repository instead of mainline - see the corresponding change in workspace.bzl.
# EIGEN_URL="$(grep -o 'http.*bitbucket.org/eigen/eigen/get/.*tar\.gz' "${BZL_FILE_PATH}" | grep -v bazel-mirror | head -n1)"
EIGEN_URL="$(grep -o 'http.*bitbucket.org/mehdi_goli/opencl/get/.*tar\.gz' "${BZL_FILE_PATH}" | grep -v bazel-mirror | head -n1)"
